...
import { ChevronDown, ChevronRight, Plus, Search, Calendar, X, Upload } from "lucide-react";
...
export default function AtestadoPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <h1 className="text-2xl font-normal text-gray-800 mb-6">Histórico de Atestados</h1>

      {!isInstrutor && (
        <div className="mb-6">
          <Button
            className="w-full bg-teal-500 hover:bg-teal-600 text-white font-semibold py-6 text-base"
      ...
      {loading ? (
        ...
        <>
          <h2 className="text-lg font-normal text-gray-800 mb-4">
            {searchActive ? `Resultados da busca (${atestados.length})` : "Últimos envios"}
          </h2>
                  ...
                  {atestados.map((atestado) => (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-gray-700 font-medium">{atestado.date}</span>
                        <span className="text-gray-500 text-sm ml-2">{atestado.status}</span>
                      </div>
                      <Plus className="h-5 w-5 text-gray-400 hover:text-teal-500" />
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="p-4 text-gray-500">Nenhum atestado encontrado no período.</p>
              )}
            </CardContent>
          </Card>
        </>
      ...
      {modalOpen && selectedAtestado && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 relative">
            ...
            <p><strong>Data:</strong> {selectedAtestado.date}</p>
            ...
            <p><strong>Foto:</strong> {selectedAtestado.fotoUrl ? <img src={selectedAtestado.fotoUrl} alt="Atestado" /> : "Sem imagem"}</p>
          </div>
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <label className="cursor-pointer flex items-center gap-2 bg-gray-200 px-3 py-2 rounded">
                  ...
                  Enviar foto
                  <input type="file" accept="image/*" onChange={(e) => setNovaFoto(e.target.files[0])} className="hidden" />
                </label>
                {novaFoto && <span>{novaFoto.name}</span>}
              </div>
              <Button className="w-full bg-teal-500 hover:bg-teal-600 text-white" onClick={handleEnviarAtestado}>
                Enviar Atestado
              </Button>
            </div>
          </div>
