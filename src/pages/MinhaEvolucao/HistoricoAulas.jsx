...
export default function HistoricoAulasPage() {
    ...
    const updateStatusSelectedAulas = async (status) => {
        try {
            const token = localStorage.getItem("token");
            await axios.put('/api/aulas/status', { ids: selectedAulas, status }, {
                headers: { Authorization: `Bearer ${token}` }
            });

            setAulas(prev => prev.map(aula => 
                selectedAulas.includes(aula.id) ? { ...aula, status, statusColor: colorMap[status] } : aula
            ));
    ...
    // Buscar aulas sempre que datas mudarem
    useEffect(() => {
        if (startDate && endDate) fetchAulas(startDate, endDate);
    }, [startDate, endDate]);

    const totalPresencas = aulas.filter(a => a.status === 'Presente').length;
    const totalFaltas = aulas.filter(a => a.status === 'Falta').length;

    if (loading) return <p className="p-4">Carregando aulas...</p>;
                        ...
                        <tr>
                            {isInstrutor && (
                                <th className="px-2 py-2 text-xs sm:text-sm">
                                    <Checkbox checked={selectedAulas.length === aulas.length} onChange={handleSelectAll} />
                                </th>
                            )}
                            ...
                            <tr key={aula.id} className="hover:bg-gray-50 transition duration-150">
                                {isInstrutor && (
                                    <td className="px-2 py-2 text-xs sm:text-sm">
                                        <Checkbox checked={selectedAulas.includes(aula.id)} onChange={() => handleSelectAula(aula.id)} />
                                    </td>
                                )}
                                ...
                                {isInstrutor && (
                                    <td className="px-2 py-2 text-xs sm:text-sm text-right relative">
                                        ...
                                        />
                                        {openDropdownId === aula.id && selectedAulas.length > 0 && (
                                            <div className="absolute right-0 mt-2 w-40 bg-white border rounded shadow-lg z-50 flex flex-col">
                                                <button className="flex items-center px-4 py-2 hover:bg-gray-100" onClick={() => updateStatusSelectedAulas("Presente")}>
                                                    <CheckCircle className="mr-2 text-green-500" />Presença
                                                </button>
                                                <button className="flex items-center px-4 py-2 hover:bg-gray-100" onClick={() => updateStatusSelectedAulas("Falta")}>
                                                    <XCircle className="mr-2 text-red-500" />Falta
                                                </button>
                                                <button className="flex items-center px-4 py-2 hover:bg-gray-100" onClick={() => updateStatusSelectedAulas("Atestado")}>
