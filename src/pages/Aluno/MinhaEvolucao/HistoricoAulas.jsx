...
export default function HistoricoAulasPage() {
    ...
    const mockData = [
      {
        ...
        data: "2025-09-02",
        observacoes: "Apresentou dificuldade inicial...",
        status: "Presente",
      ...
      },
      {
        ...
        profissional: "Ana Souza",
        data: "2025-09-03",
        observacoes: "Demonstrou interesse...",
        status: "Presente",
        statusColor: "bg-green-500 hover:bg-green-600",
      },
      {
  ...
  const updateStatusAula = async (id, status) => {
    console.log(
      `Chamando API para alterar status da aula ${id} para ${status}`
    );
    const colorMap = {
                  ...
                  <td className="px-2 py-2 text-xs sm:text-sm text-right relative">
                    ...
                    {openDropdownId === aula.id && (
                      <div className="absolute right-0 mt-2 w-40 bg-white border rounded shadow-lg z-50 flex flex-col">
                        ...
                        <button
                          className="flex items-center px-4 py-2 hover:bg-gray-100"
                          onClick={() => alterarStatus(aula.id, "Atestado")}
                        >
