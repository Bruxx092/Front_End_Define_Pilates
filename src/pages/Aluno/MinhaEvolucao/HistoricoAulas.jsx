        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {isInstrutor && (
                <th className="px-2 py-2 text-xs sm:text-sm"></th>
              )}
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {aulas.map((aula) => (
              ...
              >
                ...
                <td className="px-2 py-2 text-xs sm:text-sm max-w-xs truncate">
                  {aula.observacoes}
                </td>
                <td className="px-2 py-2 text-xs sm:text-sm">
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold text-white shadow-sm ${aula.statusColor}`}
                  >
                    {aula.status}
                  </span>
                </td>
                {isInstrutor && (
                  <td className="px-2 py-2 text-xs sm:text-sm text-right relative">
                    <MoreVertical
                      ...
                      onClick={() =>
                        setOpenDropdownId(
                          openDropdownId === aula.id ? null : aula.id
                        )
                      }
                    />
                    {openDropdownId === aula.id && selectedAulas.length > 0 && (
                      <div className="absolute right-0 mt-2 w-40 bg-white border rounded shadow-lg z-50 flex flex-col">
                        <button
                          ...
                          onClick={() => updateStatusSelectedAulas("Presente")}
                        >
                        ...
                        <button
                          className="flex items-center px-4 py-2 hover:bg-gray-100"
                          onClick={() => updateStatusSelectedAulas("Falta")}
                        >
                        ...
                        <button
                          className="flex items-center px-4 py-2 hover:bg-gray-100"
                          onClick={() => updateStatusSelectedAulas("Atestado")}
                        >
